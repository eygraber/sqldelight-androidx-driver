...
class AndroidxDriverConnectionPoolTest {
  ...
  @Test
  fun `writer mutex is released when lazy writer connection init throws`() = runTest {
    val failingFactory = object : AndroidxSqliteConnectionFactory {
    ...
    val pool = AndroidxDriverConnectionPool(
      connectionFactory = failingFactory,
      nameProvider = { "test.db" },
      isFileBased = true,
      configuration = AndroidxSqliteConfiguration(),
    )
    ...
    // If the mutex weren't released, this would suspend forever.
    assertFailsWith<IllegalStateException> {
      pool.acquireWriterConnection()
    }
  }
  ...
  private class TestConnectionFactory : AndroidxSqliteConnectionFactory {
    ...
    override suspend fun createConnection(name: String): SQLiteConnection = TestConnection()
  }
}
