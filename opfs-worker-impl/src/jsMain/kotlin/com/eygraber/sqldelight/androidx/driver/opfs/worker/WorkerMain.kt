...
private fun routeDriverMessage(e: MessageEventLike) {
  ...
  when {
    !isObject(requestMsg) || !isObject(requestMsg.data) -> {
      val id: dynamic = if(isObject(requestMsg)) requestMsg.id else null
      replyError(id, "Invalid request, missing 'data'.")
    }
    !isObject(requestMsg.data.cmd) -> replyError(requestMsg.id, "Invalid request, missing 'cmd'.")
    multiTabMode == "Shared" -> routeSharedDriverMessage(requestMsg)
    pauseState != PauseState.Live -> pausedQueue.add(e)
    else -> {
...
private fun routeSharedDriverMessage(requestMsg: dynamic) {
  if(isLeader) {
    var opaque: Int? = null
...
private fun onMessage(e: MessageEventLike) {
  ...
  when {
    !initStarted && isObject(data) && isObject(data.__opfsInit) -> onInit(data.__opfsInit)
    isObject(data) && isObject(data.__opfsControlPort) ->
      controlPorts.add(data.__opfsControlPort.unsafeCast<MessagePortLike>())
    isObject(data) && isObject(data.__opfsDebugFollowerCount) ->
      controlPorts.forEach { controlPortFollowerCount(it, followerStates.size) }
    isObject(data) && isObject(data.__opfsClose) -> requestClose()
    closeRequested -> rejectAfterClose(data)
    isObject(data) && isObject(data.__opfsPause) -> onPauseRequested()
    isObject(data) && isObject(data.__opfsResume) -> onResumeRequested()
    else -> routeOrQueueDriverMessage(e)
  }
}

private fun onInit(init: dynamic) {
  initStarted = true
  initData = init
  multiTabMode = initData.mode.unsafeCast<String?>() ?: "Single"
  when(multiTabMode) {
    "Shared" -> setupSharedMode()
    // Don't claim SAH handles until the main thread tells us we have the foreground lock.
    "PauseOnHidden" -> pauseState = PauseState.Paused
    else -> ensureLocalSqlite(
...
private fun onPauseRequested() {
  when {
    ...
    multiTabMode == "PauseOnHidden" && pauseState == PauseState.Resuming -> pendingPause = true
    multiTabMode == "PauseOnHidden" && pauseState == PauseState.Live ->
      if(anyLocalDbInTransaction()) pendingPause = true else completePause()
    else -> controlPorts.forEach(::controlPortAck)
  }
}

private fun onResumeRequested() {
  if(multiTabMode != "PauseOnHidden") return
  when(pauseState) {
    PauseState.Live -> pendingPause = false
    PauseState.Paused -> {
    ...
    PauseState.Resuming -> Unit
  }
}

private fun routeOrQueueDriverMessage(e: MessageEventLike) {
  when {
    multiTabMode == "Shared" ->
      if(acceptingDriverMessages) routeDriverMessage(e) else queuedDriverMessages.add(e)
    multiTabMode == "PauseOnHidden" ->
